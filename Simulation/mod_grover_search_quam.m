function [mod_grover_beta_iterate] = mod_grover_search_quam(phi,beta_quam)

%m = size(beta);
m_temp = size(beta_quam);
m = m_temp(1);
% l_temp = size(beta{1});
% l = l_temp(2);
n_temp = size(phi);
...
%beta_state = zeros(n,1);
  
% for p=1:m
% 
%     beta_char = beta_quam{p};
%     
%  for i=1:l
...
% phi = sqrt(1/j)*phi;
% 
% oracle = eye(2^l) - 2*beta_quam*beta_quam';
oracle = eye(n) - 2*beta_quam*beta_quam';

HIH = 2*phi0*phi0' - eye(n);
